 ...
 * Chip		: AE350_SOC
 * Function	: AE350_SOC core definitions
 * ******************************************************************************************
 */
...
#ifdef __nds_v5
// nds_intrinsic.h is in each tool-chain.
// For example, toolchains\nds32le-elf-mculib-v5\lib\gcc\riscv32-elf\10.3.0\include\nds_intrinsic.h
...
#define write_fcsr(val)     __nds__fscsr(val)
...
#define read_fcsr() ({ unsigned long __tmp; \
  asm volatile ("frcsr %0" : "=r"(__tmp)); \
  __tmp; })

#define write_fcsr(val) ({ \
  asm volatile ("fscsr %0" :: "rK"(val)); })
